...
static const char START_RELIW[] =
    ...
    "local server = ws.new(ip, port, handle.func, config.ssl)\n"
    "server.logger:set_level(config.log_level)\n"
    "server.__config.metrics_host = config.metrics_host or 'reliw.stats'\n"
    "server.__config.log_headers = config.log_headers or {}\n"
    "std.ps.setenv('RELIW_DATA_DIR', config.data_dir or '/www')\n"
    "std.ps.setenv('RELIW_REDIS_PREFIX', config.redis_prefix or 'RLW')\n"
    "std.ps.setenv('RELIW_REDIS_URL', config.redis_url or "
    "'127.0.0.1:6379/13')\n"
    "std.ps.setenv('RELIW_CACHE_MAX', config.cache_max or 5242880)\n"
    "server:serve()\n";
...
const luaL_Reg c_preload[] = {{"socket.core", luaopen_socket_core},
