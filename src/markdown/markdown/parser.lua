...
local close_paragraph = function(self)
	if self.__state.in_paragraph then
		...
		self.__state.para_content = ""
	end
end

-- Close the current list item (and paragraph inside it)
close_list_item = function(self)
	if self.__state.in_list_item then
		close_paragraph(self)
		self.__state.events:emit_block_end("list_item")
...
local close_lists_to_indent = function(self, target_indent)
	while #self.__state.list_stack > 0 do
		local current = self.__state.list_stack[#self.__state.list_stack]
		if target_indent and current.indent <= target_indent then
			break
		end
		close_list_item(self)
		self.__state.events:emit_block_end("list")
...
local close_code_block = function(self)
	if self.__state.code_block then
		...
		if self.__state.code_block.content and self.__state.code_block.content ~= "" then
			self.__state.events:emit_text(self.__state.code_block.content)
		end
		self.__state.events:emit_block_end("code_block")
		self.__state.code_block = nil
	end
end
...
local close_all_divs = function(self)
	while #self.__state.div_stack > 0 do
		close_div(self)
	end
end
...
close_blockquote = function(self)
	if #self.__state.blockquote_stack > 0 then
		...
		close_all_lists(self)
		close_all_divs(self)
		self.__state.events:emit_block_end("blockquote")
	end
end

-- Close all open blockquotes
close_all_blockquotes = function(self)
	while #self.__state.blockquote_stack > 0 do
...
local handle_div_line = function(self, line)
	...
	local current_div = self.__state.div_stack[#self.__state.div_stack]
	...
	if fence_len then
		close_paragraph(self)
		-- Open nested div
		self.__state.div_stack[#self.__state.div_stack + 1] = {
			fence_len = fence_len,
			class = class_name,
			indent = indent,
		}
		...
		self.__state.events:emit_block_start("div", attrs)
		return true -- Line consumed
	end
...
local handle_code_block_line = function(self, line)
	local cb = self.__state.code_block
...
close_table = function(self)
	if not self.__state.table then
		return
	end

	local tbl = self.__state.table

	-- Emit table events
	self.__state.events:emit_block_start("table", { columns = #tbl.alignments })

	-- Emit header row
	self.__state.events:emit_block_start("table_head")
	self.__state.events:emit_block_start("table_row", { header = true })
	for i, cell in ipairs(tbl.header_cells) do
		self.__state.events:emit_block_start("table_cell", { align = tbl.alignments[i] or "left", header = true })
		-- Parse inline content in cell
		if self.cfg.parse_inline then
			local parser = inline.new({
				emit = function(e)
					self.__state.events:emit(e)
				end,
			...
			self.__state.events:emit_text(cell)
		end
		self.__state.events:emit_block_end("table_cell")
	end
	self.__state.events:emit_block_end("table_row")
	self.__state.events:emit_block_end("table_head")

	-- Emit body rows
	if #tbl.body_rows > 0 then
		self.__state.events:emit_block_start("table_body")
		for _, row_cells in ipairs(tbl.body_rows) do
			self.__state.events:emit_block_start("table_row")
			for i, cell in ipairs(row_cells) do
				self.__state.events:emit_block_start("table_cell", { align = tbl.alignments[i] or "left" })
				-- Parse inline content in cell
				if self.cfg.parse_inline then
					local parser = inline.new({
						emit = function(e)
		...
		self.__state.events:emit_block_end("table_body")
	end

	self.__state.events:emit_block_end("table")
	self.__state.table = nil
end
...
local handle_table_line = function(self, line)
	...
	if cells then
		...
		local col_count = #self.__state.table.alignments
		...
		self.__state.table.body_rows[#self.__state.table.body_rows + 1] = cells
		return true -- Line was consumed
	end
...
local handle_blank_line = function(self)
	...
	if #self.__state.list_stack > 0 then
		...
		self.__state.pending_blank = true
	end
end
...
local emit_inline_content = function(self, content)
	...
	if self.cfg.streaming_inline and self.__state.inline_buffer then
		-- Streaming mode: feed to buffer for incremental emission
		self.__state.inline_buffer:feed(content)
	else
...
local emit_softbreak = function(self)
	if self.cfg.streaming_inline and self.__state.inline_buffer then
		...
		self.__state.inline_buffer:feed(" ")
	else
		self.__state.events:emit_softbreak()
	end
end

-- Open a new heading
local open_heading = function(self, attrs)
	self.__state.events:emit_block_start("heading", { level = attrs.level })
	...
	if self.cfg.parse_inline then
		...
		self.__state.events:emit_text(attrs.content)
	end

	self.__state.events:emit_block_end("heading")
end
...
local open_code_block = function(self, attrs)
	...
	local list_indent = 0
	if #self.__state.list_stack > 0 then
		list_indent = self.__state.list_stack[#self.__state.list_stack].content_indent
	end
	self.__state.code_block = {
	...
	self.__state.events:emit_block_start("code_block", block_attrs)
end

-- Emit a thematic break
local emit_thematic_break = function(self)
	self.__state.events:emit_block_start("thematic_break")
	self.__state.events:emit_block_end("thematic_break")
end
...
local function start_list(self, attrs)
	...
	table.insert(self.__state.list_stack, list)
	self.__state.events:emit_block_start("list", {
...
local function start_list_item(self, attrs, close_previous)
	...
	self.__state.events:emit_block_start("list_item", item_attrs)
...
process_line = function(self, line)
	...
	if bq_content ~= nil then
		...
		if self.__state.current_footnote then
			self.__state.current_footnote = nil
			self.__state.footnote_indent = nil
		end
		...
		if #self.__state.blockquote_stack == 0 then
			...
			close_table(self)
			self.__state.blockquote_stack[#self.__state.blockquote_stack + 1] = { indent = bq_indent }
			self.__state.events:emit_block_start("blockquote")
		end
		...
		self.__state.processing_blockquote_content = true
		process_line(self, bq_content)
		self.__state.processing_blockquote_content = false
		return
	elseif #self.__state.blockquote_stack > 0 and not self.__state.processing_blockquote_content then
	...
	if not self.__state.in_paragraph and #self.__state.list_stack == 0 and not self.__state.table then
		local label, dest, title = state.detect_link_reference(line)
		if label then
			-- Store link reference (first definition wins)
			if not self.__state.link_refs[label] then
				self.__state.link_refs[label] = { url = dest, title = title }
			end
	...
	if not self.__state.in_paragraph and #self.__state.list_stack == 0 and not self.__state.table then
		...
		if label then
			...
			if not self.__state.footnotes[label] then
				self.__state.footnotes[label] = { content = content, lines = {} }
				self.__state.current_footnote = label
				self.__state.footnote_indent = indent
			end
			return -- Line consumed
		end
	end
	...
	if self.__state.current_footnote then
		...
		if #leading_spaces >= self.__state.footnote_indent + 2 then
			-- Continuation of footnote
			local content = line:sub(self.__state.footnote_indent + 2 + 1) -- Strip indent
			table.insert(self.__state.footnotes[self.__state.current_footnote].lines, content)
			return -- Line consumed
		elseif state.is_blank_line(line) then
			-- Blank line in footnote - check next line
			table.insert(self.__state.footnotes[self.__state.current_footnote].lines, "")
			return -- Line consumed
		else
			-- End of footnote
			self.__state.current_footnote = nil
	...
	if not self.__state.in_paragraph and not self.__state.table then
		local fence_len, class_name, indent = state.detect_div_fence_open(line)
		if fence_len then
			close_paragraph(self)
			close_all_lists(self)
	...
	if self.__state.pending_table_row then
		local pending = self.__state.pending_table_row
		self.__state.pending_table_row = nil
	...
	self.__state.pending_blank = false
	...
	if #self.__state.list_stack > 0 then
		...
		for i = #self.__state.list_stack, 1, -1 do
			local list = self.__state.list_stack[i]
...
local feed = function(self, chunk)
	...
	local content = self.__state.pending .. chunk
	...
	if last_newline < #content then
		self.__state.pending = content:sub(last_newline + 1)
	else
...
local finish = function(self)
	...
	if self.__state.pending ~= "" then
		process_line(self, self.__state.pending)
		self.__state.pending = ""
	end
	...
	if self.__state.footnote_tracker and self.__state.footnote_tracker.used then
		for label, _ in pairs(self.__state.footnote_tracker.used) do
			if self.__state.footnotes[label] then
	...
	if #used_footnotes > 0 then
		...
		self.__state.events:emit_block_start("footnotes")
		for i, label in ipairs(used_footnotes) do
			local fn = self.__state.footnotes[label]
			...
			self.__state.events:emit_block_start("footnote", { label = label, index = i })
			...
			if self.cfg.parse_inline then
				...
				self.__state.events:emit_text(full_content)
			end
			self.__state.events:emit_block_end("footnote")
		end
		self.__state.events:emit_block_end("footnotes")
	end
end
...
local reset = function(self)
	...
	for k in pairs(self.__state.link_refs) do
		self.__state.link_refs[k] = nil
	end
	self.__state.footnotes = {}
	for k in pairs(self.__state.footnote_tracker.used) do
		self.__state.footnote_tracker.used[k] = nil
	end
	self.__state.current_footnote = nil
	self.__state.footnote_indent = nil
	...
	self.__state.processing_blockquote_content = false
	if self.__state.inline_buffer then
		self.__state.inline_buffer:reset()
	end
end

local set_event_callback = function(self, on_event)
	local callback = on_event or function() end
	self.cfg.on_event = callback
	self.__state.events:set_callback(callback)
	if self.__state.inline_buffer then
		self.__state.inline_buffer:set_emit(callback)
	end
end
...
local new = function(options)
	...
	local events_emitter = events.new(emit_callback)
	...
	return {
		cfg = {
			on_event = emit_callback,
			parse_inline = parse_inline,
			streaming_inline = streaming_inline,
		},
		__state = {
			events = events_emitter,
			pending = "",
			in_paragraph = false,
			code_block = nil,
			para_content = "",
			...
			current_footnote = nil, -- Currently collecting footnote
			footnote_indent = nil, -- Indent level of current footnote
			div_stack = {}, -- Stack of open divs
			blockquote_stack = {}, -- Stack of open blockquotes
			processing_blockquote_content = false,
		},
		feed = feed,
		finish = finish,
		reset = reset,
