...
static int create(lua_State *L) {
  p_context ctx;
  const char *mode_str =
      luaL_optstring(L, 1, "client"); // Default to client mode
  ...
  printf("Context created successfully with mode: %s\n",
         mode_str); // Debug print
...
static int set_sni(lua_State *L) {
  ...
  ret = wolfSSL_CTX_UseSNI(ctx, WOLFSSL_SNI_HOST_NAME, server_name,
                           strlen(server_name));
  if (ret != SSL_SUCCESS) {
    lua_pushboolean(L, 0);
...
static int load_locations(lua_State *L) {
  ...
  if (certfile && keyfile) {
    int ret =
        wolfSSL_CTX_use_certificate_file(ctx, certfile, WOLFSSL_FILETYPE_PEM);
    if (ret != 1) {
      const char *err =
          wolfSSL_ERR_reason_error_string(wolfSSL_get_error(ctx, ret));
      lua_pushboolean(L, 0);
      lua_pushfstring(L, "error loading server certificate (%s): %s", certfile,
                      err);
      return 2;
    }
  ...
  lua_pushboolean(L, 1);
  printf("Certificates and keys loaded successfully\n"); // Debug print
  return 1;
}
 ...
 * Package functions
 */
static luaL_Reg funcs[] = {{"create", create},
 ...
 * Context metamethods.
 */
static luaL_Reg meta[] = {{"__close", meth_destroy},
                          {"__gc", meth_destroy},
 ...
 * Index metamethods.
 */
static luaL_Reg meta_index[] = {{"setverifyext", meth_set_verify_ext},
                                {NULL, NULL}};
