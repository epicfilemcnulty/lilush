 ...
 */
static int lsec_socket_error() { return errno; }
...
static WOLFSSL_CTX *check_client_sni(p_ssl ssl, const unsigned char *buffer,
                                     size_t len) {
  ...
  int ret = wolfSSL_SNI_GetFromBuffer(
      buffer,
...
static int meth_create(lua_State *L) {
  ...
  io_init(&ssl->io, (p_send)ssl_send, (p_recv)ssl_recv, (p_error)ssl_ioerror,
          ssl);
...
static int meth_add_sni_context(lua_State *L) {
  ...
  sni_list_entry *new_entries =
      realloc(ssl->sni_contexts->entries,
              (ssl->sni_contexts->count + 1) * sizeof(sni_list_entry));
  ...
  ssl->sni_contexts->entries[ssl->sni_contexts->count].servername =
      strdup(servername);
 ...
 * SSL methods
 */
static luaL_Reg methods[] = {{"close", meth_close},
                             {"getfd", meth_getfd},
 ...
 * SSL functions.
 */
static luaL_Reg funcs[] = {{"create", meth_create},
                           {"setfd", meth_setfd},
