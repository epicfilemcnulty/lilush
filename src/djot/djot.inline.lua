...
function InlineParser.between_matched(c, annotation, defaultmatch, opentest)
	return function(self, pos, endpos)
		...
		local has_close_marker = pos + 1 <= endpos and byte(subject, pos + 1) == 125 -- }
		local endcloser = pos
...
InlineParser.matchers = {
	...
	[96] = function(self, pos, endpos)
		...
		if find(subject, "^%$%$", pos - 2) and not find(subject, "^\\", pos - 3) then
	...
	[92] = function(self, pos, endpos)
		...
		if endchar then
			...
			if #self.matches > 0 then
				...
				if annot == "str" then
					while ep >= sp and (subject:byte(ep) == 32 or subject:byte(ep) == 9) do
						ep = ep - 1
					end
					if ep < sp then
	...
	[60] = function(self, pos, endpos)
		local subject = self.subject
		local starturl, endurl = bounded_find(subject, "^%<[^<>%s]+%>", pos, endpos)
		if starturl then
	...
	[41] = function(self, pos, endpos)
		...
		if parens and #parens > 0 and parens[#parens][1] then
			...
			local subject = self.subject
			local openers = self.openers["["]
			if openers and #openers > 0 and openers[#openers][3] == "explicit_link" then
				local opener = openers[#openers]
				-- we have inline link
				local is_image = bounded_find(subject, "^!", opener[1] - 1, endpos)
					and not bounded_find(subject, "^[\\]", opener[1] - 2, endpos)
	...
	-- 43 = +
	[43] = InlineParser.between_matched("+", "insert", "str", function(self, pos)
		return find(self.subject, "^%{", pos - 1) or find(self.subject, "^%}", pos + 1)
	end),

	-- 61 = =
	[61] = InlineParser.between_matched("=", "mark", "str", function(self, pos)
		return find(self.subject, "^%{", pos - 1) or find(self.subject, "^%}", pos + 1)
	end),

	-- 39 = '
	[39] = InlineParser.between_matched("'", "single_quoted", "right_single_quote", function(self, pos) -- test to open
		return pos == 1 or find(self.subject, "^[%s\"'-([]", pos - 1)
	end),
	...
	[45] = function(self, pos, endpos)
		local subject = self.subject
		local nextpos
		if byte(subject, pos - 1) == 123 or byte(subject, pos + 1) == 125 then -- (123 = { 125 = })
			nextpos = InlineParser.between_matched("-", "delete", "str", function(slf, p)
				return find(slf.subject, "^%{", p - 1) or find(slf.subject, "^%}", p + 1)
			end)(self, pos, endpos)
			return nextpos
		end
			...
			elseif self.verbatim > 0 then
				if c == 96 then
					...
					if endchar and endchar - pos + 1 == self.verbatim then
						...
						local sp, ep = bounded_find(subject, "^%{%=[^%s{}`]+%}", endchar + 1, endpos)
