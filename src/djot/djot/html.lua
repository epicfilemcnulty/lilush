...
local insert_attribute, copy_attributes = ast.insert_attribute, ast.copy_attributes
...
function Renderer:render_attrs(node)
	if node.attr then
		for k, v in pairs(node.attr) do
			self.out(" " .. k .. "=" .. '"' .. self:escape_html_attribute(v) .. '"')
		end
	end
	if node.pos then
		local sp, ep = unpack(node.pos)
		self.out(' data-startpos="' .. tostring(sp) .. '" data-endpos="' .. tostring(ep) .. '"')
	end
end
...
function Renderer:reference_definition() end
