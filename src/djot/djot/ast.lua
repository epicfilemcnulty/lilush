...
local find, lower, sub, rep, format = string.find, string.lower, string.sub, string.rep, string.format
...
local function new_attributes(tbl)
	...
	setmetatable(attr, {
		__pairs = sortedpairs(function(a, b)
			return a < b
		end, function(k)
			return k
		end),
...
local function to_ast(parser, sourcepos)
	...
	local function get_identifier(s)
		local base = s:gsub("[][~!@#$%^&*(){}`,.<>\\|=+/?]", ""):gsub("^%s+", ""):gsub("%s+$", ""):gsub("%s+", "-")
		local i = 0
		local ident = base
		-- generate unique id
	...
	local function format_sourcepos(bytepos)
		if bytepos then
			return string.format(
				"%d:%d:%d",
				sourceposmap.line[bytepos],
				sourceposmap.col[bytepos],
				sourceposmap.charpos[bytepos]
			)
		end
	end
	...
	local function is_tight(startidx, endidx, is_last_item)
		...
		for i = startidx, endidx do
			...
			if tag == "blankline" then
				if
					not (
						string.find(tags[i + 1], "%+list_item")
						or (
							string.find(tags[i + 1], "%-list_item")
							and (is_last_item or string.find(tags[i + 2], "%-list_item"))
						)
					)
				then
	...
	local function add_child_to_tip(containers, child)
		if
			containers[#containers].t == "list"
			and not (child.t == "list_item" or child.t == "definition_list_item")
		then
			-- close list
		...
		if child.t == "list" then
			...
			local tight = true
			for i = 1, #child.c do
				tight = tight and is_tight(child.c[i].startidx, child.c[i].endidx, i == #child.c)
			...
			if tag == containers[#containers].t then
				...
				elseif tag == "caption" then
					...
					if prevnode and prevnode.t == "table" then
						...
						warn({ message = "Ignoring caption without preceding table", pos = startpos })
					end
					return
				elseif tag == "heading" then
					local heading_str = get_string_content(node):gsub("^%s+", ""):gsub("%s+$", "")
					...
					if not references[heading_str] then
						references[heading_str] = new_node("reference")
				...
				elseif tag == "destination" then
					...
					local prevnode = has_children(tip) and tip.c[#tip.c]
					assert(
						prevnode and (prevnode.t == "image" or prevnode.t == "link"),
				...
				assert(false, "unmatched " .. annot .. " encountered at byte " .. startpos)
				return
			end
		else
			-- process leaf node:
