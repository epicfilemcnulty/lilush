...
function Parser:parse_table_row(sp, ep)
	...
	while not sepfound do
		local sepsp, sepep, left, right, trailing = find(self.subject, "^(%:?)%-%-*(%:?)([ \t]*%|[ \t]*)", p)
		if sepep then
	...
	return {
...

		{
			...
			open = function(spec)
				...
				if ep then
					self.pos = ep + 1
					self:add_container(
						Container:new(spec, { inline_parser = InlineParser:new(self.subject, self.warn) })
					)
...

		{
			...
			open = function(spec)
				...
				local data = { styles = styles, indent = self.indent }
				-- adding container will close others
...

		{
			...
			open = function(spec)
				...
				if ep and self.starteol == ep + 1 then
					self:add_container(Container:new(spec, { key = label, indent = self.indent }))
			...
			open = function(spec)
				...
				if ep and find(self.subject, "^%s", ep + 1) then
					...
					self:add_container(
						Container:new(
							spec,
							{ level = level, inline_parser = InlineParser:new(self.subject, self.warn) }
						)
					)
...

		{
			...
			continue = function(container)
				...
				local sp, ep, border = self:find("^(" .. container.border .. char .. "*)[ \t]*[\r\n]")
				if ep then
			...
			open = function(spec)
				local sp, ep, border, ws, lang = self:find("^(~~~~*)([ \t]*)(%S*)[ \t]*[\r\n]")
				if not ep then
					sp, ep, border, ws, lang = self:find("^(````*)([ \t]*)([^%s`]*)[ \t]*[\r\n]")
				end
				if border then
...

		{
			...
			open = function(spec)
				if self:find("^%{") then
					...
					local status, ep = attribute_parser:feed(self.pos, self.endeol)
			...
			continue = function(container)
				if self.indent > container.indent then
					...
					local status, ep = container.attribute_parser:feed(self.pos, self.endeol)
				...
				if container.status == "done" then
					...
					local para = Container:new(para_spec, { inline_parser = InlineParser:new(self.subject, self.warn) })
...
function Parser:get_inline_matches()
	local matches = self.containers[#self.containers].inline_parser:get_matches()
	for i = 1, #matches do
...
function Parser:add_container(container)
	...
	while
		#self.containers > last_matched
		or (#self.containers > 0 and self.containers[#self.containers].content ~= "block")
	do
	...
	return function() -- iterator
		while self.pos <= subjectlen do
			...
			if not self.finished_line then
				...
				local check_starts = not is_blank
					and (not last_match or last_match.content == "block")
					and not self:find("^%a+%s") -- optimization
				...
				if not self.finished_line then
					...
					local is_lazy = not is_blank
						and not new_starts
						and self.last_matched_container < #self.containers
						and self.containers[#self.containers].content == "inline"
...
return { Parser = Parser, Container = Container }
