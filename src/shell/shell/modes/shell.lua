...
local settings = function(self, combo)
	...
	term.set_raw_mode()
	term.hide_cursor()
	widgets.settings(self.conf, "Shell Settings", theme.widgets.settings.shell, theme.widgets.switcher.shell, 3, 5)
...
local python_env = function(self, cmd, args)
	...
	if cmd == "activate" then
		...
		if virtual_env == "" and base_dir then
			...
			term.set_raw_mode()
			term.hide_cursor()
			local choice = widgets.switcher(content, theme.widgets.switcher.python)
			term.switch_screen("main")
			term.show_cursor()
			term.go(l, c)
			term.set_sane_mode()
