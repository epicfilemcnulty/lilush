...
local try_set_socket_option = function(self, server, option, value)
	local ok, err = server:setoption(option, value)
	if ok then
		return true
	end
	self.logger:log({
		msg = "failed to set socket option; continuing",
		option = option,
		error = tostring(err),
...
local server_process_request = function(self, client, client_ip, count)
	...
	client:settimeout(self.cfg.keepalive_idle_timeout)
	...
	if headers["transfer-encoding"] then
		client:settimeout(self.cfg.request_body_timeout)
		body, err = read_chunked_body(client, self.cfg.max_body_size)
		if err then
			if err == "timeout" then
	...
	if content_length > 0 and not headers["transfer-encoding"] then
		if content_length > self.cfg.max_body_size then
			premature_error(client, 413, "A body too fat\n")
	...
	if headers["accept-encoding"] and headers["accept-encoding"]:match("deflate") and self.cfg.compression.enabled then
	...
	if content and #content > 0 then
		if
			compress_output
			and self.cfg.compression.types[response_headers["content-type"]]
			and #content >= self.cfg.compression.min_size
		then
	...
	if count >= self.cfg.requests_per_fork then
	...
	if self.logger:level() <= 10 then
		...
		for _, h in ipairs(self.cfg.log_headers) do
			if headers[h] then
				log_msg[h] = headers[h]
			end
		end
...
local server_serve = function(self)
	...
	try_set_socket_option(self, server, "reuseaddr", true)
	try_set_socket_option(self, server, "reuseport", true)
	local ok
	ok, err = server:bind(self.cfg.ip, self.cfg.port)
	if not ok then
		safe_close(server)
		return nil,
			"failed to bind " .. tostring(self.cfg.ip) .. ":" .. tostring(self.cfg.port) .. ": " .. tostring(err)
	end
	ok, err = server:listen(self.cfg.backlog)
	if not ok then
		safe_close(server)
		return nil,
			"failed to listen on " .. tostring(self.cfg.ip) .. ":" .. tostring(self.cfg.port) .. ": " .. tostring(err)
	end
	ok, err = server:settimeout(0)
	if not ok then
		safe_close(server)
		return nil, "failed to configure server timeout: " .. tostring(err)
	end
	...
	self.logger:log({
		...
		fork_limit = self.cfg.fork_limit,
		requests_per_fork = self.cfg.requests_per_fork,
	...
	while true do
		...
		if not timeout then
			...
			elseif server_fork_count >= self.cfg.fork_limit then
				local now = os.time()
				...
				elseif pid > 0 then
					...
					if self.cfg.ssl then
						...
						if self.__state.ssl_contexts.hosts then
							for hostname, ctx in pairs(self.__state.ssl_contexts.hosts) do
								ssl_client:add_sni_context(hostname, ctx)
							end
						end

						ssl_client:settimeout(self.cfg.tls_handshake_timeout)
					...
					repeat
						...
						count = count + 1
					until state == "close" or count > self.cfg.requests_per_fork
					safe_close(ssl_client)
					safe_close(client)
					os.exit(0)
				end
...
local server_configure = function(self, config)
	...
	self.cfg = std.tbl.merge(self.cfg, config)
	self.logger:set_level(self.cfg.log_level)
	if self.cfg.ssl then
		-- Create default context
		if self.cfg.ssl.default then
			if
				not std.fs.file_exists(self.cfg.ssl.default.cert)
				or not std.fs.file_exists(self.cfg.ssl.default.key)
			then
			...
			local cfg = {
				mode = "server",
				keyfile = self.cfg.ssl.default.key,
				certfile = self.cfg.ssl.default.cert,
			}
			local ctx, err = ssl.newcontext(cfg)
			if not ctx then
				return nil, "failed to create default SSL context: " .. err
			end
			self.__state.ssl_contexts.default = ctx
		end

		-- Create contexts for additional hosts
		if self.cfg.ssl.hosts then
			self.__state.ssl_contexts.hosts = {}
			for domain, ssl_config in pairs(self.cfg.ssl.hosts) do
				...
				self.__state.ssl_contexts.hosts[domain] = ctx
			end
		end
	end
	return true
